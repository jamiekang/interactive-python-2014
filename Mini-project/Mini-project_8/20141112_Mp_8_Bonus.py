...
class Sprite:
    ...
    def collide(self, other_object):
        ...
        distance = dist(pos_self, pos_other)
...
def draw(canvas):
    global time, started, lives, score, rock_group, my_ship, explosion_group
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12:
        ...
        dist_rock = dist(rock_pos, ship_pos)
        if dist_rock >= MINDIST:
            rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
            score_scale = 1 + (score / SCALEFACTOR)
            rock_vel = [rock_vel[0] * score_scale, rock_vel[1] * score_scale]
...
def group_group_collide(group, other_group):
    collision_count = 0
    for each in set(group):
        if group_collide(other_group, each):
            group.discard(each)
            collision_count += 1
...
def group_collide(group, other_object):
    ...
    for each in set(group):
        if each.collide(other_object):
            new_explosion = Sprite(other_object.pos, other_object.vel, 0, 0, explosion_image, explosion_info, explosion_sound)
